...
(function ($, gmb) {
	...
	gmb.get_editable_info_window = function( index, marker ) {
		...
		info_window_content += '<div class="infowindow-toolbar clear"><ul id="save-toolbar">' +
			...
			'</ul>' +
			'<span class="marker-edit-link-wrap" data-index="' + index + '"><a href="#" data-target="marker-icon-modal" aria-label="Change icon" data-mfp-src="#marker-icon-modal" class="marker-edit-link gmb-magnific-marker gmb-magnific-inline hint--top"><span class="dashicons dashicons-location"></span> Customize Icon</a></span>' +
			'</div>';
		...
		var save_icon_listener = google.maps.event.addDomListener( $( '.save-marker-button' )[ 0 ], 'click', function( e ) {
			...
			var marker_label_data = '<i class="' + $( this ).data( 'label' ) + '" style="' + marker_label_inline_style + '" data-pin="'+ marker_icon +'"></i>';
			...
			var marker_args = {
				...
				map_icon_label: marker_label_data,
	...
	gmb.create_search_result_marker = function (place) {
		...
		var search_marker = new mapIcons.Marker({
			map: map,
			zIndex: 0,
			optimized: false
		});
	...
	gmb.calc_radius = function (map, radiusVal) {
		...
		//update marker with set marker
		radius_marker = new mapIcons.Marker({
